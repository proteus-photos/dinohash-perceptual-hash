...
class AdamWSP(Optimizer):
    ...
    def __init__(
        ...
        super(AdamWSP, self).__init__(params, defaults)

        self.pretrained = []
        for group in self.param_groups:
            self.pretrained.append([p.clone().detach() for p in group['params']])


    def __setstate__(self, state):
        super(AdamWSP, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group, pretrained_group in zip(self.param_groups, self.pretrained):
            params_with_grad = []
            pretrained_params = []
            grads = []
            exp_avgs = []
            ...
            for p, pretrained_p in zip(group['params'], pretrained_group):
                if p.grad is None:
                    continue
                params_with_grad.append(p)
                pretrained_params.append(pretrained_p)
                if p.grad.is_sparse:
...
def _single_tensor_adamw(
    ...
    for i, (param, pretrain_param) in enumerate(zip(params, pretrained_params)):
        ...
        param.sub_(wd_scale * weight_decay * (param-pretrain_param))
...
def _multi_tensor_adamw(
    ...
    difference = torch._foreach_sub(params, pretrained_params)
    torch._foreach_mul_(difference, wd_scale * weight_decay)
    torch._foreach_add_(params, difference)
